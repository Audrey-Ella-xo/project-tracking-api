...
#   let(:project_id) { projects.first.id }
  ...
  get '/projects/:id' do
    ...
    example_request 'Getting a specific project' do
      explanation 'Shows an specific project no authentication is needed'
      expect(status).to eq(200)
    end
  end
  ...
  post '/projects' do
    ...
    example_request 'Creating a new project' do
      ...
      do_request(title: 'Squats', description: 'Squating', created_by: user.id.to_s)
  ...
  put '/projects/:id' do
    ...
    example_request 'Updating an specific project' do
      ...
      do_request(title: 'Push-ups', description: 'Pushing up', created_by: user.id.to_s)
